...
export default function AuthButton() {
  ...
  const handleSignIn = async () => {
    ...
    // Direct access: normal OAuth flow
  ...
  return (
    <div className="space-y-3">
      <button
        ...
        className="flex items-center space-x-2 px-4 py-2 bg-white border border-gray-300 rounded-lg shadow-sm hover:shadow-md transition-all duration-200 text-gray-700 hover:text-gray-900 w-full justify-center"
      >
        ...
        <span className="text-sm font-medium">
          {isInIframe ? 'Google로 로그인 (팝업)' : 'Google로 로그인'}
        </span>
      </button>
      {isInIframe && (
        <div className="text-xs text-gray-500 text-center p-2 bg-green-50 rounded border border-green-200">
          🚀 팝업으로 로그인 후 이 iframe에서 계속 사용할 수 있습니다
        </div>
      )}
    </div>
  )
